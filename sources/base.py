...
from sources.buffers import (
    CircularBufferQueue,
    CircularResultsBufferQueue
)

from sources.utils.locator import (
...
class BaseReader():
    ...
    def record_start(self, filename):
        ...
        if not os.path.exists(os.path.dirname(filename)):
            ...
            filename = os.path.join(
                basedir, "data", os.path.basename(filename))
    ...
    def convert_data_to_int16(self, data):
        ...
        for index in range(num_samples):
            ...
            struct.pack_into(
                "<" + "h", sample_data, index *
                2, int(tmp[index] * self.scaling_factor)
            )
